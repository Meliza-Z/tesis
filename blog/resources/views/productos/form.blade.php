...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ isset($producto->id) ? route('productos.update', $producto) : route('productos.store') }}"
                    method="POST">
                    @csrf
                    ...
                    <div id="campos-producto" class="campos-ocultos"
                        style="{{ old('categoria', $producto->categoria ?? '') ? 'display: block;' : 'display: none;' }}">

                        <!-- Mensaje de instrucción cuando no hay categoría seleccionada -->
                        <div id="mensaje-categoria" class="alert alert-info"
                            style="{{ old('categoria', $producto->categoria ?? '') ? 'display: none;' : 'display: block;' }}">
                            <i class="fas fa-info-circle me-2"></i>
                            <strong>Selecciona una categoría</strong> para continuar completando la información del
                            producto.
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="precio_base" class="form-label">Precio <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <span class="input-group-text">$</span>
                                        <input type="text" name="precio_base" id="precio_base"
                                            class="form-control @error('precio_base') is-invalid @enderror"
                                            value="{{ old('precio_base', isset($producto->precio_base_centavos) ? number_format($producto->precio_base_centavos / 100, 2, '.', '') : '') }}"
                                            placeholder="0.00" required>
                                    </div>
                                    @error('precio_base')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="text-muted">Usa punto (.) o coma (,) para decimales. Ej: 10.50 o
                                        10,50</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="descripcion" id="descripcion" class="form-control @error('descripcion') is-invalid @enderror"
                                rows="4" placeholder="Describe las características principales del producto...">{{ old('descripcion', $producto->descripcion ?? '') }}</textarea>
                            @error('descripcion')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Función para validar el formato de precio
            function isValidPrice(price) {
                // Permitir formato con punto o coma como separador decimal
            ...
            // Función para normalizar el precio antes de enviar
            function normalizePriceInput() {
                const priceInput = document.getElementById('precio_base');
                if (priceInput.value) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (precio === '' || !isValidPrice(precio)) {
                    e.preventDefault();
                    alert('Por favor, ingresa un precio válido (ej: 10.50 o 10,50).');
                    document.getElementById('precio_base').focus();
                    return false;
                }
