...
@section('content')
    <div class="container-fluid">
            ...
            <div class="col-12 d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">
                        Límite: ${{ number_format($limiteCentavos / 100, 2) }} · Usado:
                        ${{ number_format($expuestoCentavos / 100, 2) }}
                    </p>
                </div>
                <div>
        ...
        <div class="card shadow-sm mb-4">
            <div
                class="card-header d-flex justify-content-between align-items-center {{ $bloqueado ? 'bg-secondary' : 'bg-primary' }} text-white">
                <h6 class="mb-0"><i class="fas fa-plus me-2"></i>Crear nuevo crédito</h6>
                @if ($bloqueado)
                    <span class="badge bg-dark">Bloqueado
                        ({{ $tieneVencido ? 'tiene vencido' : 'límite de 2 abiertos' }})</span>
                @else
                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="date" name="fecha_credito" id="fecha_credito" class="form-control"
                                value="{{ old('fecha_credito', now()->toDateString()) }}" required
                                {{ $bloqueado ? 'disabled' : '' }}>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Fecha vencimiento</label>
                            <input type="date" name="fecha_vencimiento" id="fecha_vencimiento" class="form-control"
                                value="{{ old('fecha_vencimiento') }}" required {{ $bloqueado ? 'disabled' : '' }}>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Plazo (días)</label>
                            <input type="number" name="plazo_dias" id="plazo_dias" class="form-control"
                                value="{{ old('plazo_dias', 15) }}" min="1" {{ $bloqueado ? 'disabled' : '' }}>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Extensión (opcional)</label>
                            <input type="date" name="fecha_vencimiento_ext" id="fecha_vencimiento_ext"
                                class="form-control" value="{{ old('fecha_vencimiento_ext') }}"
                                {{ $bloqueado ? 'disabled' : '' }}>
                        ...
                        <tbody>
                            @forelse($creditos as $c)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge bg-{{ $c->estado === 'pagado' ? 'success' : ($c->estado === 'vencido' ? 'danger' : 'secondary') }}">
                                            {{ ucfirst($c->estado) }}
                                        </span>
                                    </td>
                                </tr>
